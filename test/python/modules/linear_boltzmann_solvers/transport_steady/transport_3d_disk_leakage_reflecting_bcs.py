...
Q = 122.58 particles/s, with reflecting boundaries on the top and bottom and a vacuum boundary on
the cylindrical side.

For a single energy group, pure absorber with sigma_t = 1.0 m^-1, the total leakage can be
approximated as: L = Q * <exp(-sigma_t * s)> where s is the distance to the side wall after any
number of reflections in z. The average leakage fraction can be evaluated numerically as
<exp(-sigma_t * s)> ~= 0.7930.

Hence, total leakage ~= 122.58 * 0.7930 = 97.20 particles/s.

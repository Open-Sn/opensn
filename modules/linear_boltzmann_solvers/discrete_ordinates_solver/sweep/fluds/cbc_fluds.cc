...
CBC_FLUDS::LocalUpwindDataBlock() const
{
  return local_psi_data_;
}

const double*
CBC_FLUDS::LocalCellUpwindPsi(const std::vector<double>& psi_data_block, const Cell& cell)
{
...
const std::vector<double>&
CBC_FLUDS::NonLocalUpwindData(uint64_t cell_global_id, unsigned int face_id) const
{
  return deplocs_outgoing_messages_.at({cell_global_id, face_id});
}

const double*
CBC_FLUDS::NonLocalUpwindPsi(const std::vector<double>& psi_data,

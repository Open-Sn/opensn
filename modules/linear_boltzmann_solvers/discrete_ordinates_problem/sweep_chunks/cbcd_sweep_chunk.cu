...
struct Index
{
  ...
  /// Index of the cell associated to the current thread in the current set of ready cells.
  unsigned int cell_idx;
  /// Index of the angle associated to the current thread in the current angleset.
  unsigned int angle_idx;
  /// Index of the group associated to the current thread in the current groupset.
...
CBCDSweepChunk::GPUSweep(CBCD_AngleSet& angle_set, const std::vector<std::uint64_t>& cell_local_ids)
{
  ...
  auto* device_saved_psi = fluds->GetDeviceSavedPsi().get();

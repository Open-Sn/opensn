...
__global__ void
UnsarturatedKernel(Arguments args, const std::uint32_t* level, double* saved_psi)
{
...
/// Kernel performing the sweep
__global__ void
SarturatedKernel(Arguments args, const std::uint32_t* level, double* saved_psi)
{
  // reference to indexes
  const unsigned int& cell_idx = blockIdx.x;
  // get cell view
...
AAHSweepChunk::GPUSweep(AngleSet& angle_set)
{
  ...
  {
    MeshCarrier* mesh_carrier_ptr = reinterpret_cast<MeshCarrier*>(problem_.GetCarrier(2));
    saved_psi =
  ...
  if (block_size_x * block_size_y <= gpu_kernel::threshold)
  {
    for (std::uint32_t level = 0; level < levelized_spls.size(); ++level)
    {
      ...
      gpu_kernel::UnsarturatedKernel<<<level_size, {block_size_x, block_size_y}>>>(
  ...
  else
  {
    for (std::uint32_t level = 0; level < levelized_spls.size(); ++level)
    {
      ...
      std::size_t level_size = levelized_spls[level].size();
      const std::uint32_t* level_data = spds.GetDeviceLevelVector(level);
      gpu_kernel::SarturatedKernel<<<level_size, 256>>>(args, level_data, saved_psi.get());
    }
  }
  ...
  if (save_angular_flux_)
  {
    ...
    MeshCarrier& mesh_carrier = *(reinterpret_cast<MeshCarrier*>(problem_.GetCarrier(2)));

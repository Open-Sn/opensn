                                 ...
                                 bool is_surface_source_active)
{
  ...
  MeshCarrier* mesh = reinterpret_cast<MeshCarrier*>(problem.GetCarrier(2));
  mesh_data = mesh->GetDevicePtr();
  QuadratureCarrier* quadrature = reinterpret_cast<QuadratureCarrier*>(groupset.quad_carrier);
  quad_data = quadrature->GetDevicePtr();
  // copy source moment and destination phi data to GPU
  MemoryPinner<double>* src = reinterpret_cast<MemoryPinner<double>*>(problem.GetPinner(0));
  src->CopyToDevice();
  ...
  MemoryPinner<std::uint32_t>* directions_num =

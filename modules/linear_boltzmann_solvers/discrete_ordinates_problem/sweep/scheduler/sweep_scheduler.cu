...
SweepScheduler::DeviceScheduleAlgoFIFO(SweepChunk& sweep_chunk)
{
  ...
  while (executed_anglesets < num_angle_sets)
  {
    ...
    for (size_t i = 0; i < num_angle_sets; ++i)
    {
      ...
      const auto& stream = std::any_cast<const crb::Stream&>(angle_sets[i]->GetStream());
      ...
      if (stream.is_completed())
      {
        ...
        fluds.CopyOutgoingBoundaryAndNonLocalPsiToHost(sweep_chunk, in_flight_tasks[i]);
  ...
  // Copy back saved angular fluxes if requested
  if (cbc_sweep_chunk.IsSavingAngularFluxes())
    cbc_sweep_chunk.CopySavedPsiFromDevice(angle_sets);

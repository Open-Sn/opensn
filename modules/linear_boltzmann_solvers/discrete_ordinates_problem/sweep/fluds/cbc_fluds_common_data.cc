...
constexpr int FACE_SLOT_TAG = 102;
    ...
    num_delayed_local_face_nodes_(0)
{
  ...
  for (const auto& cell : grid.local_cells)
  {
    ...
    for (std::size_t f = 0; f < cell.faces.size(); ++f)
    {
      ...
      {
        ++num_outgoing_faces_;
        ++outgoing_face_counts_by_location_[face.GetNeighborPartitionID(&grid)];
      }
    }
  }
  ...
  if (allow_cycles)
  {
    ...
    delayed_face_flags_.assign(num_local_faces, 0);
  }
  ...
  if (allow_cycles)
  {
    delayed_dependency_index_by_location.assign(static_cast<std::size_t>(opensn::mpi_comm.size()),
                                                INVALID_FACE_SLOT);
    ...
    for (std::size_t f = 0; f < cell.faces.size(); ++f)
    {
      ...
      if (face.has_neighbor and face.IsNeighborLocal(&grid))
      {
        ...
        const bool delayed_incoming =
          allow_cycles and orientation == FaceOrientation::INCOMING and
          cbc_spds.IsDelayedLocalDependency(adj_cell.local_id, cell.local_id);
        const bool delayed_outgoing =
          allow_cycles and orientation == FaceOrientation::OUTGOING and
      ...
      if (orientation == FaceOrientation::INCOMING)
      {
        ...
        auto& records = incoming_slot_records_by_upstream_location_[neighbor_location];
...
CBC_FLUDSCommonData::FinalizeBeta()
{
  ...
  // Each outgoing face receives one record, including faces delayed by the downstream rank.
  std::map<int, std::vector<std::uint64_t>> downstream_slot_records;
  for (const auto& [location, face_count] : outgoing_face_counts_by_location_)
  {
    if (face_count >
        static_cast<std::size_t>(std::numeric_limits<int>::max()) / FACE_SLOT_RECORD_SIZE)
      throw std::overflow_error("CBC FLUDS receive record count exceeds MPI int range");
    downstream_slot_records[location].resize(face_count * FACE_SLOT_RECORD_SIZE);
  }
  for (const auto& [location, records] : incoming_slot_records_by_upstream_location_)
    if (records.size() > static_cast<std::size_t>(std::numeric_limits<int>::max()))
      throw std::overflow_error("CBC FLUDS send record count exceeds MPI int range");

  // Ordered finalization and exact-source matching allow tag reuse across SPDS objects.
  std::vector<mpi::Request> receives(downstream_slot_records.size());
  std::vector<mpi::Request> sends(incoming_slot_records_by_upstream_location_.size());
  std::size_t request = 0;
  for (auto& [location, records] : downstream_slot_records)
    receives[request++] =
      mpi_comm.irecv(location, FACE_SLOT_TAG, records.data(), static_cast<int>(records.size()));
  request = 0;
  for (const auto& [location, records] : incoming_slot_records_by_upstream_location_)
    sends[request++] =
      mpi_comm.isend(location, FACE_SLOT_TAG, records.data(), static_cast<int>(records.size()));
  mpi::wait_all(receives);
  mpi::wait_all(sends);

  incoming_slot_records_by_upstream_location_.clear();
  outgoing_face_counts_by_location_.clear();
  ...
  for (const auto& location_records : downstream_slot_records)
  {
    ...
    {
      const CellFaceKey key{records[i], static_cast<unsigned int>(records[i + 1])};
      const RemoteFaceSlot slot{static_cast<std::size_t>(records[i + 2]), records[i + 3] != 0};
  ...
  for (const auto& cell : grid.local_cells)
  {
    ...
    for (std::size_t f = 0; f < cell.faces.size(); ++f)
    {
      ...
      const auto& remote_slot = downstream_slot_by_face.at(key);
...
{
  return not delayed_face_flags_.empty() and
         (delayed_face_flags_[face_offsets_[cell_local_id] + face_id] & LOCAL_INCOMING);
}
...
{
  return not delayed_face_flags_.empty() and
         (delayed_face_flags_[face_offsets_[cell_local_id] + face_id] & LOCAL_OUTGOING);
}
...
{
  return not delayed_face_flags_.empty() and
         (delayed_face_flags_[face_offsets_[cell_local_id] + face_id] & NONLOCAL_INCOMING);
}
...
{
  return not delayed_face_flags_.empty() and
         (delayed_face_flags_[face_offsets_[cell_local_id] + face_id] & NONLOCAL_OUTGOING);
}

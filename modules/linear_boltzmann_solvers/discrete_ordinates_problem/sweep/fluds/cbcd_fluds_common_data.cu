...
CBCD_FLUDSCommonData::CopyCellFaceNodeMapToDevice()
{
  ...
  for (const auto& cell : grid.local_cells)
    for (std::uint32_t f = 0; f < cell.faces.size(); ++f)
      total_face_nodes += sdm_.GetCellMapping(cell).GetNumFaceNodes(f);
  ...
  for (const auto& cell : grid.local_cells)
  {
    ...
    current_dof_offset += sdm_.GetCellMapping(cell).GetNumNodes();
  }
  ...
  for (const auto& cell : grid.local_cells)
  {
    ...
    for (size_t f = 0; f < cell.faces.size(); ++f)
    {
      ...
      const size_t num_face_nodes = sdm_.GetCellMapping(cell).GetNumFaceNodes(f);
...
CBCD_FLUDSCommonData::DeallocateDeviceCellFaceNodeMap()
{
  if (device_cell_face_node_map_ != nullptr)
  {
    ...
    device_cell_face_node_map.reset(device_cell_face_node_map_);

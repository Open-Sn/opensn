 ...
 */
struct DelayedLocalBank : public AAHD_Bank
{
  /// @name Constructors
  /// @{
  /// @brief Default constructor.
  DelayedLocalBank() = default;
  /// @brief Member constructor.
  DelayedLocalBank(std::size_t size, std::size_t num_groups_and_angles)
 ...
 */
struct BoundaryBank : public AAHD_Bank
{
  /// @name Constructors
  /// @{
  /// @brief Default constructor.
  ...
  /// @brief Member constructor.
  BoundaryBank(std::size_t size, std::size_t num_groups_and_angles)
    : AAHD_Bank(size * num_groups_and_angles)
  {
  }
  /// @}
 ...
 */
struct NonLocalBank : public AAHD_Bank
{
  /// @name Constructors
  /// @{
  /// @brief Default constructor.
  NonLocalBank() = default;
  /// @brief Member constructor.
  NonLocalBank(const std::vector<std::size_t>& sizes, std::size_t num_groups_and_angles);
  /// @}

  /// @name Copy and move
  /// @{
  /// @brief Copy constructor.
  NonLocalBank(const NonLocalBank& other);
  /// @brief Copy assignment.
  NonLocalBank& operator=(const NonLocalBank& other);
  /// @brief Move constructor.
  NonLocalBank(NonLocalBank&& other) = default;
  /// @brief Move assignment.
  NonLocalBank& operator=(NonLocalBank&& other) = default;
  /// @}
...
public:
  ...
  /// @name Allocate memory for banks
  /// @{
  ...
  /// @name Sweep preparation and clean up
  /// @{
...
protected:
  ...
  DelayedLocalBank delayed_local_psi_bank_;
  /// Delayed local bank for old angular fluxes.
  DelayedLocalBank delayed_local_psi_old_bank_;

  /// Non-local bank for incoming angular fluxes.
  NonLocalBank nonlocal_incoming_psi_bank_;
  /// Non-local bank for delayed incoming angular fluxes.
  NonLocalBank nonlocal_delayed_incoming_psi_bank_;
  /// Non-local bank for old delayed incoming angular fluxes.
  NonLocalBank nonlocal_delayed_incoming_psi_old_bank_;
  /// Non-local bank for outgoing angular fluxes.
  NonLocalBank nonlocal_outgoing_psi_bank_;

  /// Device storage for boundary angular fluxes.
  BoundaryBank boundary_psi_;

             ...
             bool use_gpus)
{
  ...
  std::vector<std::shared_ptr<SPDS>> result(work.size());
  const size_t nthreads = std::min<std::size_t>(work.size(), std::max(1U, opensn_num_threads));
  log.Log() << program_timer.GetTimeString() << " SPDS construction: " << work.size() << " angles ("
            << nthreads << " thread(s)).";
...
UnpackCBCSPDSLocationEdgeWeights(const std::vector<std::uint64_t>& packed_values)
{
  constexpr std::size_t packed_values_per_edge = 3;
...
AccumulateCBCGlobalEdgeWeights(const std::vector<std::shared_ptr<CBC_SPDS>>& spds_list)
{
  ...
  for (size_t spds_ordinal = 0; spds_ordinal < spds_list.size(); ++spds_ordinal)
  {
    ...
    if (opensn::mpi_comm.rank() == owner)
      spds_list[spds_ordinal]->SetGlobalEdgeWeights(
        UnpackCBCSPDSLocationEdgeWeights(global_values));
  }
}
...
BuildCBCLocalFaceSlotPlan(SweepRuntime& runtime)
{
  ...
  const auto nthreads = std::min<std::size_t>(spds_list.size(), std::max(1U, opensn_num_threads));

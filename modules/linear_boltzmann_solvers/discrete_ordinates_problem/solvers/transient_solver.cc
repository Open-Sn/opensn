...
TransientSolver::StepPrecursors()
{
  ...
  for (std::uint32_t cell_local_id = 0; cell_local_id < grid->GetLocalCellCount(); ++cell_local_id)
  {
    ...
    const auto& fe_values = do_problem_->GetUnitCellMatrices()[cell_local_id];
    const double cell_volume = transport_views[cell_local_id].GetVolume();
    ...
    for (int i = 0; i < transport_views[cell_local_id].GetNumNodes(); ++i)
    {
      const size_t uk_map = transport_views[cell_local_id].MapDOF(i, 0, 0);
    ...
    {
      const size_t dof_map = cell_local_id * max_precursors + j;

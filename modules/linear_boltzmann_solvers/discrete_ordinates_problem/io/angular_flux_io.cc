  ...
  const std::vector<std::pair<std::string, std::pair<std::string, double>>>& int_surfs)
{
  ...
  if (not bndry_surfs.empty())
  {
    ...
    for (const auto& bndry : bndry_surfs)
    {
      ...
      surface_tags.insert(allowed_bd_ids.at(bndry_id));
    }
  }

  for (const auto& [name, definition] : int_surfs)
  {
    const auto& axis = definition.first;
    OpenSnInvalidArgumentIf(axis != "x" and axis != "y" and axis != "z",
                            "Invalid internal-surface axis '" + axis + "'.");
    surface_tags.insert(name + "_u");
    surface_tags.insert(name + "_d");
  }
  ...
  for (size_t groupset_index = 0; groupset_index < groupsets.size(); ++groupset_index)
  {
    ...
    for (const auto& surf_id : surface_tags)
    {
      if (groupset_index == 0)
      {
        const auto& cell_ids = cell_map[surf_id];
        const auto& num_face_nodes = node_map[surf_id];
        const auto& x_surf = x_map[surf_id];
        const auto& y_surf = y_map[surf_id];
        const auto& z_surf = z_map[surf_id];
        ...
        for (size_t f = 0; f < cell.faces.size() and not compatible_face_found; ++f)
        {
          ...
          for (const auto& file_node : file_nodes)
          {
            ...
            for (size_t fi = 0; fi < num_face_nodes; ++fi)
            {
              ...
              if ((cell_mapping.GetNodeLocations()[node] - file_node).NormSquare() < 1.0e-12)
              {

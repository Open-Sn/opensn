...
RequireCurvilinearProductQuadrature(const LBSGroupset& groupset)
{
  ...
  if (quadrature == nullptr)
    throw std::invalid_argument("CBCSweepChunkRZ::CBCSweepChunkRZ: invalid angular quadrature");
  return *quadrature;
}
...
CBCSweepChunkRZ::CBCSweepChunkRZ(DiscreteOrdinatesProblem& problem, LBSGroupset& groupset)
  : SweepChunk(problem.GetPhiNewLocal(),
    ...
    sweep_impl_(&CBCSweepChunkRZ::Sweep_Dispatch)
{
...
void
CBCSweepChunkRZ::Sweep_Dispatch(AngleSet& angle_set)
{
  // Uniform meshes bind a fixed-N or generic kernel during construction.
  switch (discretization_.GetCellMapping(*cell_).GetNumNodes())
  {
    case 2:
...
CBCSweepChunkRZ::Sweep_Generic(AngleSet& angle_set)
{
  ...
  if (source_.empty())
  {
    streaming_matrix_.resize({max_num_cell_dofs_, max_num_cell_dofs_});
    system_matrix_.resize({max_num_cell_dofs_, max_num_cell_dofs_});
    rhs_.assign(gs_size, Vector<double>(max_num_cell_dofs_));
    source_.resize(max_num_cell_dofs_);
    polar_dof_map_.resize(max_num_cell_dofs_);
  }
  auto& Amat = streaming_matrix_;
  auto& Atemp = system_matrix_;
  auto& b = rhs_;
  auto& source = source_;
  auto& polar_dof_map = polar_dof_map_;
  ...
  for (std::size_t as_ss_idx = 0; as_ss_idx < num_angles_in_as; ++as_ss_idx)
  {
    ...
    for (std::size_t f = 0; f < cell_num_faces; ++f)
    {
      ...
      const bool incident_on_symmetric_boundary =
        is_boundary_face and face.normal.Dot(normal_vector_boundary_) < -0.999999;

    ...
    sweep_impl_(&AAHSweepChunkRZ::Sweep_Generic)
{
  ...
  const std::size_t dir_map_size = curvilinear_product_quadrature->GetDirectionMap().size();
...
AAHSweepChunkRZ::Sweep_FixedN(AngleSet& angle_set)
{
  ...
  for (std::size_t spls_index = 0; spls_index < num_spls; ++spls_index)
  {
    ...
    OpenSnInvalidArgumentIf(cell_mapping.GetNumNodes() != static_cast<std::size_t>(NumNodes),

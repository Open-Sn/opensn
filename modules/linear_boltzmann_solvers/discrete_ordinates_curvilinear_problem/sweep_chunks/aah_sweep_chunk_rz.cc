...
RequireCurvilinearProductQuadrature(const LBSGroupset& groupset)
{
  ...
  if (quadrature == nullptr)
    throw std::invalid_argument("AAHSweepChunkRZ::AAHSweepChunkRZ: invalid angular quadrature");
  return *quadrature;
}
    ...
    sweep_impl_(&AAHSweepChunkRZ::Sweep_Generic)
{
  ...
  const std::size_t dir_map_size = direction_map.size();
  for (std::size_t m = 0; m < dir_map_size; ++m)
...
AAHSweepChunkRZ::Sweep_Generic(AngleSet& angle_set)
{
  ...
  std::vector<std::uint64_t> polar_dof_map(max_num_cell_dofs_);

  // Loop over each cell
  const auto& spds = angle_set.GetSPDS();
  const auto& spls = spds.GetLocalSubgrid();
  const std::size_t num_spls = spls.size();
  for (std::size_t spls_index = 0; spls_index < num_spls; ++spls_index)
  {
    ...
    for (std::size_t as_ss_idx = 0; as_ss_idx < as_angle_indices.size(); ++as_ss_idx)
    {
      ...
      const auto fac_diamond_difference =
        curvilinear_quadrature_.GetDiamondDifferenceFactor()[direction_num];
      const auto fac_streaming_operator =
        curvilinear_quadrature_.GetStreamingOperatorFactor()[direction_num];
      ...
      for (std::size_t i = 0; i < cell_num_nodes; ++i)
        polar_dof_map[i] = discretization_.MapDOFLocal(cell, i, unknown_manager_, polar_level, 0);

      for (std::size_t i = 0; i < cell_num_nodes; ++i)
      {
        for (std::size_t j = 0; j < cell_num_nodes; ++j)
        {
          const auto jr = polar_dof_map[j];
          for (std::size_t gsg = 0; gsg < gs_size; ++gsg)
      ...
      for (std::size_t f = 0; f < cell_num_faces; ++f)
      {
        ...
        const auto& surface_matrix = M_surf[f];
        const std::size_t num_face_nodes = cell_mapping.GetNumFaceNodes(f);
        const double mu_f = -face_mu_values[f];

        // Determine whether the incoming direction is incident on the point of symmetry
        // or on the axis of symmetry.
        // N.B.: A face is considered to be on the point/axis of symmetry if all are true:
        //  1. The face normal is antiparallel to $\vec{e}_{d}$.
        //  2. All vertices of the face exhibit $v_{d} = 0$, where $d = 2$ for 1D geometries
        //     and $d = 0$ for 2D geometries.
        // Verifications are performed during initialization, so it is sufficient to
        // to confirm only the orientation.
        const bool incident_on_symmetric_boundary =
          is_boundary_face and cell_face.normal.Dot(normal_vector_boundary_) < -0.999999;
        ...
        for (std::size_t fi = 0; fi < num_face_nodes; ++fi)
        {
          ...
          for (std::size_t fj = 0; fj < num_face_nodes; ++fj)
          {
            ...
            else if (not incident_on_symmetric_boundary)
            ...
            const double mu_Nij = mu_f * surface_matrix(i, j);
            Amat(i, j) += mu_Nij;
      ...
      for (std::size_t f = 0; f < cell_num_faces; ++f)
      {
        ...
        for (std::size_t fi = 0; fi < num_face_nodes; ++fi)
        {
          ...
          for (size_t gsg = 0; gsg < gs_size; ++gsg)
            cell_outflow_view.Add(f, gs_gi + gsg, flux_i * b[gsg](i));
...
AAHSweepChunkRZ::Sweep_FixedN(AngleSet& angle_set)
{
  ...
  std::vector<std::array<std::size_t, NumNodes>> moment_dof_map(num_moments_);
  ...
  for (std::size_t spls_index = 0; spls_index < num_spls; ++spls_index)
  {
    ...
    for (std::size_t as_ss_idx = 0; as_ss_idx < as_angle_indices.size(); ++as_ss_idx)
    {
      ...
      std::array<std::uint64_t, NumNodes> polar_dof_map{};
      PRAGMA_UNROLL
      for (std::size_t i = 0; i < NumNodes; ++i)
        polar_dof_map[i] = discretization_.MapDOFLocal(cell, i, unknown_manager_, polar_level, 0);

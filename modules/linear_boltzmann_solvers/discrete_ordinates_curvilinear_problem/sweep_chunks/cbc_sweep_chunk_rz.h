...
private:
  ...
  /// Dense buffers allocated on the first generic sweep and reused across cells.
  DenseMatrix<double> streaming_matrix_;
  DenseMatrix<double> system_matrix_;
  std::vector<Vector<double>> rhs_;
  std::vector<double> source_;
  /// Reusable polar-level node addresses for generic sweeps.
  std::vector<std::uint64_t> polar_dof_map_;

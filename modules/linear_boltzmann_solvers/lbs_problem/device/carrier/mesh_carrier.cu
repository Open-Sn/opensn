  ...
  *(num_cell_data++) = num_cells;
  ...
  for (char* cell_data = data; const Cell& cell : mesh.local_cells)
  {
    ...
    *(phi_address_data++) = phi_address;
    ...
    // offset for save angular flux
    std::uint64_t* save_psi_index_data = reinterpret_cast<std::uint64_t*>(cell_data);
    *(save_psi_index_data++) = save_psi_index;
    saved_psi_offset.push_back(save_psi_index);
    save_psi_index += cell_num_nodes;
    cell_data = reinterpret_cast<char*>(save_psi_index_data);
  ...
  num_nodes_total = save_psi_index;
}

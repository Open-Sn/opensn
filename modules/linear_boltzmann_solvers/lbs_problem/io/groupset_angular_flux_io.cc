...
{
  assert(groupset_id >= 0 and groupset_id < lbs_problem.GetGroupsets().size());
  ...
  std::vector<double>& src =
    opt_src.has_value() ? opt_src.value().get() : lbs_problem.GetPsiNewLocal().at(groupset_id);
...

{
  ...
  std::vector<double>& dest =
    opt_dest.has_value() ? opt_dest.value().get() : lbs_problem.GetPsiNewLocal().at(groupset_id);

...
class CMakeExtension(Extension):
    ...
    This extension takes a source directory containing a ``CMakeLists.txt`` file as an input to process.
    """
...
class CMakeBuilder(build_ext):
    ...
    Build Python extension from a ``CMakeExtension`` in the build directory. Built binaries are then be copied to either
    the source directory (inplace build mode) or the library directory (pip install mode).
    """
    ...
    def build_extension(self, ext: CMakeExtension) -> None:
        ...
        cmake_args = [
            ...
            f"-DOPENSN_WITH_LUA=OFF",
            f"-DOPENSN_WITH_PYTHON=ON"
        ]
        ...
        if "CMAKE_ARGS" in os.environ:
            cmake_args += [extra_args for extra_args in os.environ["CMAKE_ARGS"].split(" ") if extra_args]

  ...
  if (property_name == "boundary_type")
  {
    ...
    else if (type_name == "robin")
    {
      if (num_args != 7)
      {
        opensn::Chi::log.Log0Error()
          ...
          << " chiDFEMDiffusionsetBCproperty(...,\"boundary_type\"," << bound_name << ",\"robin\". "
    ...
    else
    {
      opensn::Chi::log.LogAllError()
        ...
        << "chiDFEMDiffusionSetBCProperty(..,\"boundary_type\",.. :" << type_name;
      opensn::Chi::Exit(EXIT_FAILURE);
    }
  }
  else
  {
    opensn::Chi::log.Log0Error() << "Invalid property in chiDFEMDiffusionSetBCProperty.";
    opensn::Chi::Exit(EXIT_FAILURE);
  }
  return 0;
}

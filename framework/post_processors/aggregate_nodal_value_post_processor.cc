...
AggregateNodalValuePostProcessor::Execute(const Event& event_context)
{
  ...
  if (operation_ == "max")
  {
    ...
    MPI_Allreduce(&local_max_value, &globl_max_value, 1, MPI_DOUBLE, MPI_MAX, mpi_comm);
  ...
  else if (operation_ == "min")
  {
    ...
    MPI_Allreduce(&local_min_value, &globl_min_value, 1, MPI_DOUBLE, MPI_MIN, mpi_comm);
  ...
  else if (operation_ == "avg")
  {
    ...
    MPI_Allreduce(&local_accumulation, &globl_accumulation, 1, MPI_DOUBLE, MPI_SUM, mpi_comm);

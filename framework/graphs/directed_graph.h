...
public:
  ...
  public:
    ...
    /**
     * Adds a vertex to the graph where the ID is assigned to the number of vertices already loaded
     * on the graph. For example, if there are 3 vertices on the graph (with IDs 0 through 2) then
     * the next vertex (this one) will be assigned and ID of 3.
     */
  ...
  /// Adds a vertex to the graph. By default <I>context</I> is assumed to be nullptr.
  void AddVertex(size_t id, void* context = nullptr);

  /**
   * Adds a vertex to the graph. By default <I>context</I> is  assumed to be nullptr and <I>id</I>
   * is assumed to be assigned automatically. In the latter case the vertex id will be the same as
   * the order in which it was added (0,1,2,3,etc ... will have id's 0,1,2,3,etc)
   */
...
private:
  /**
   * Depth-First-Search main recursive algorithm. This is the recursive portion of the method below
   * this one (opensn::DirectedGraph::DepthFirstSearch).
   */
  void DFSAlgorithm(std::vector<size_t>& traversal, std::vector<bool>& visited, size_t cur_vid);

  /**
   * SCC main recursive algorithm. This is the recursive call for the method defined below this one
...
public:
  /**
   * Find strongly connected components. This method is the implementation of Tarjan's algorithm
   * [1].
   *
   * [1] Tarjan R.E. "Depth-first search and linear graph algorithms",
   *     SIAM Journal on Computing, 1972.
   *
   * It returns collections of vertices that form strongly connected components excluding
   * singletons.
   */
   ...
   *
   * \return Returns the vertex ids sorted topologically. If this vector is empty the
   *         algorithm failed because it detected cyclic dependencies.
   */
  std::vector<size_t> GenerateTopologicalSort();

  /**
   * Finds a sequence that minimizes the Feedback Arc Set (FAS). This algorithm implements
   * the algorithm depicted in [1].
   *
   * [1] Eades P., Lin X., Smyth W.F., "Fast & Effective heuristic for the feedback arc set
   *     problem", Information Processing Letters, Volume 47. 1993.
   */

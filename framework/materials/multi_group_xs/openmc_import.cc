                             ...
                             double temperature)
{
  ...
  // Number of groups
  if (not H5ReadAttribute<std::size_t>(file, "energy_groups", mgxs.num_groups_))
  ...
  if (H5Has(file, path + "scatter_data/scatter_matrix"))
  {
    ...
    int fidx = 0;
    for (std::size_t gp = 0; gp < mgxs.num_groups_; ++gp)
  ...
  if (mgxs.is_fissionable_)
  {
    ...
    for (std::size_t g = 0; g < mgxs.num_groups_; ++g)

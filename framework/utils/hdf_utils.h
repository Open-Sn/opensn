...
{
public:
  explicit H5FileHandle(const hid_t file_id) : file_id_(file_id) {}

  H5FileHandle(const H5FileHandle&) = delete;
  H5FileHandle& operator=(const H5FileHandle&) = delete;

  H5FileHandle(H5FileHandle&& other) noexcept : file_id_(other.file_id_)
  {
...
template <typename T>
bool H5ReadAttribute(hid_t id, const std::string& name, T& value);
...
template <typename T>
bool
H5ReadOptionalAttribute(hid_t id, const std::string& name, T& value)
{
  if (H5Aexists(id, name.c_str()) <= 0)
    return true;
  return H5ReadAttribute<T>(id, name, value);
}

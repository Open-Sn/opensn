 ...
 *
 * \tparam Element type
 * \param dest 2-D NDArray to copy into
 * \param row Row index into the `dest`
 * \param src Vector that will be copied
 */
template <typename T>
void
MemCopyRow(NDArray<T, 2>& dest, unsigned int row, const std::vector<T>& src)
{
  assert(dest.dimension()[1] == src.size());
  auto ofst = dest.MapNDtoLin(row, 0);
  std::memcpy(dest.data() + ofst, src.data(), sizeof(T) * src.size());
}

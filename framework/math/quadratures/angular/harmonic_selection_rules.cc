...
HarmonicSelectionRules::SelectHarmonics(const SelectionParameters& params)
{
  ...
  {
    case AngularQuadratureType::PRODUCT_QUADRATURE:
    ...
    case AngularQuadratureType::TRIANGULAR_QUADRATURE:
      if (params.dimension == 2)
    ...
    case AngularQuadratureType::LEBEDEV_QUADRATURE:
      return SelectLebedev(params);

    case AngularQuadratureType::SLDFE_SQ:
...
HarmonicSelectionRules::Select2DCartesianProduct(const SelectionParameters& params)
{
  ...
  for (unsigned int L = 0; L <= max_L; ++L)
  {
    ...
    for (unsigned int M = min_M; M <= max_M; M += 2)
    {
      ...
      Logger::GetInstance().Log0Verbose2()
        << "    Accepted: ell=" << L << ", m=" << -static_cast<int>(M);
      if (harmonics.size() == num_dir)
        break;
      ...
      {
        harmonics.emplace_back(L, static_cast<int>(M));
        Logger::GetInstance().Log0Verbose2() << "    Accepted: ell=" << L << ", m=" << M;
        if (harmonics.size() == num_dir)
          break;
      }
    }
    ...
    if (L <= L_crit and ind_L == 2)
    {
      harmonics.emplace_back(L, 0);
      Logger::GetInstance().Log0Verbose2() << "    Accepted: ell=" << L << ", m=" << 0;
      if (harmonics.size() == num_dir)
        break;
    }
  ...
  Logger::GetInstance().Log0Verbose1() << "2D Product (General): Selected " << harmonics.size()
...
HarmonicSelectionRules::Select2DTriangular(const SelectionParameters& params)
{
  ...
  while (harmonics.size() < nb_dir)
  {
    ...
    if (mode == EE and nb_C[EE] < ndir_oct)
    {
      harmonics.emplace_back(L, 0);
      nb_C[EE]++;
    ...
    int Mcur = ind_L;
    while (Mcur <= L and harmonics.size() < nb_dir)
    {
      // cosine (L, +M)
      ...
      // sine (L, -M)
      if (nb_S[mode] < ndir_oct and harmonics.size() < nb_dir)
      {
        harmonics.emplace_back(L, -Mcur);
        nb_S[mode]++;
        Logger::GetInstance().Log0Verbose2()
...
HarmonicSelectionRules::SelectLebedev(const SelectionParameters& params)
{
  ...
  // Warn if GALERKIN_ONE or GALERKIN_THREE is requested for orders known to lack a full
  // orthogonal basis (orders 13, 25, 27 have fewer orthogonal vectors than quadrature points)
  static const std::set<int> galerkin_unsupported_orders = {13, 25, 27};
  if ((params.construction_method == OperatorConstructionMethod::GALERKIN_ONE or
       params.construction_method == OperatorConstructionMethod::GALERKIN_THREE) and
      galerkin_unsupported_orders.count(lebedev_order))
  {
    Logger::GetInstance().Log0Warning()
      << "Galerkin method requested for Lebedev order " << lebedev_order
      << ", but this order does not have a complete set of orthogonal vectors. "
      << "The resulting operators may be incorrect. Consider using a different order.";
  }
  ...
  if (order_num != lebedev_harmonics.end())
  {
    ...
    {
      // Only include harmonics with ell <= scattering_order
      if (ell <= params.scattering_order)
      {
        harmonics.emplace_back(ell, m);
        Logger::GetInstance().Log0Verbose2() << "    Accepted: ell=" << ell << ", m=" << m;
      }
      else
      {
        Logger::GetInstance().Log0Verbose2()
          << "    Rejected (ell > scattering_order): ell=" << ell << ", m=" << m;
      }
    }
...
HarmonicSelectionRules::SelectSLDFESQ(const SelectionParameters& params)
{
  ...
  auto refinement_level = static_cast<int>(params.quadrature_order);
...
HarmonicSelectionRules::SelectStandard(const SelectionParameters& params)
{
  ...
  Logger::GetInstance().Log0Verbose1()
    << "HarmonicSelectionRules::SelectStandard - STANDARD method";
  Logger::GetInstance().Log0Verbose1()
    << "  Dimension: " << params.dimension << ", Scattering order: " << params.scattering_order;
  ...
  {
    Logger::GetInstance().Log0Verbose1() << "  2D Harmonic selection:";
    for (unsigned int ell = 0; ell <= params.scattering_order; ++ell)
    {
      std::stringstream ss;
      ss << "    ell=" << ell << ": m = [";
      ...
      ss << "]";
      Logger::GetInstance().Log0Verbose1() << ss.str();
    }
  }
  else if (params.dimension == 3)
  {
  ...
  Logger::GetInstance().Log0Verbose1() << "  Total harmonics selected: " << harmonics.size();

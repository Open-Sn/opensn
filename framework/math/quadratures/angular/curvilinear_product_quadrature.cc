...
OpenSnRegisterObjectInNamespace(aquad, GLCProductQuadrature2DRZ);

InputParameters
GLCProductQuadrature2DRZ::GetInputParameters()
{
  InputParameters params;

  params.SetGeneralDescription("2D, RZ geometry, Gauss-Legendre-Chebyshev product quadrature");
...
GLCProductQuadrature2DRZ::Create(const ParameterBlock& params)
{
  auto& factory = opensn::ObjectFactory::GetInstance();
  return factory.Create<GLCProductQuadrature2DRZ>("aquad::GLCProductQuadrature2DRZ", params);
}

GLCProductQuadrature2DRZ::GLCProductQuadrature2DRZ(const InputParameters& params)
  : CurvilinearQuadrature(2)
{
  ...
  if (param_count != 2)
  {
    throw std::invalid_argument(
      "GLCProductQuadrature2DRZ: Both Npolar and Nazimuthal must be specified");
  }
...
void
GLCProductQuadrature2DRZ::Initialize(const int Npolar, const int Nazimuthal, const bool verbose)
{
  const auto quad_polar = GaussLegendreQuadrature(Npolar, verbose);
  std::vector<GaussQuadrature> azimu_quad_vec;
  for (auto n = 0; n < Npolar; ++n)
    azimu_quad_vec.emplace_back(GaussChebyshevQuadrature(Nazimuthal, verbose));
  ...
  if (quad_polar.weights.size() != azimu_quad_vec.size())

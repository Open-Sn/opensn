    ...
    num_vals_(x_values_.size())
{
  ...
  OpenSnInvalidArgumentIf(y_values_.size() < 2, "Number of pairs must at least be 2");
  ...
  for (size_t k = 0; k < max_k; ++k)
  {
    ...
    OpenSnInvalidArgumentIf(delta_x_values_[k] <= 0.0,
...
PiecewiseLinear1D::ScalarFunction1Parameter(double x) const
{
  ...
  OpenSnLogicalError("Bad trouble");
}

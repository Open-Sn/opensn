...
chi_math::QuadratureTriangle::QuadratureTriangle(QuadratureOrder order) : Quadrature(order)
{
  ...
  switch (order)
  {
    ...
    case QuadratureOrder::FOURTH:
    {
      ...
      const double wts[n_wts] = {double(1.1169079483900573284750350421656140e-01L),
                                 double(5.4975871827660933819163162450105264e-02L)};
    ...
    case QuadratureOrder::FIFTH:
    {
      ...
      // to generate additional precision.
      //
      // Note that the following 7th-order Ro3-invariant rule also has only 12
      // points, which technically makes it the superior rule.  This one is here
      // for completeness.
    case QuadratureOrder::SIXTH:
    {
      const unsigned int n_wts = 3;
      const Real wts[n_wts] = {Real(5.8393137863189683012644805692789721e-02L),
                               Real(2.5422453185103408460468404553434492e-02L),
                               Real(4.1425537809186787596776728210221227e-02L)};

      const Real a[n_wts] = {Real(2.4928674517091042129163855310701908e-01L),
    ...
    case QuadratureOrder::SEVENTH:
    {
      ...
      // In each of the rows below, the first two entries are (z1, z2) which
      // imply z3.  The third entry is the weight for each of the points in the
      // cyclic permutation. The original publication tabulated about 16 decimal
      // digits for each point and weight parameter. The additional digits shown
      // here were obtained using a code in the mp-quadrature library,
      // https://github.com/jwpeterson/mp-quadrature
      const Real rule_data[nrows][3] = {
        {Real(6.2382265094402118173683000996350e-02L),
         Real(6.7517867073916085442557131050869e-02L),
         Real(2.6517028157436251428754180460739e-02L)}, // group A
        {Real(5.5225456656926611737479190275645e-02L),
         Real(3.2150249385198182266630784919920e-01L),
         Real(4.3881408714446055036769903139288e-02L)}, // group B
        {Real(3.4324302945097146469630642483938e-02L),
         Real(6.6094919618673565761198031019780e-01L),
         Real(2.8775042784981585738445496900219e-02L)}, // group C
        {Real(5.1584233435359177925746338682643e-01L),
         Real(2.7771616697639178256958187139372e-01L),
         Real(6.7493187009802774462697086166421e-02L)} // group D
      };
      ...
      //       // The following is an inferior 7th-order Lyness-style rule with
      //       15 points.
      //       // It's here only for completeness and the Ro3-invariant rule
      //       above should
      //       // be used instead!
      //       const unsigned int n_wts = 3;
      ...
      // };
      //
      //       const unsigned int permutation_ids[n_wts] = {3, 6, 6}; // 15
      //       total points
      //
      //       dunavant_rule2(wts, a, b, permutation_ids, n_wts);
      //
      ...
      // Another Dunavant rule.  This one has all positive weights.  This rule
      // has 16 points while a comparable conical product rule would have
      // 5*5=25.
      //
      ...
      // on triangles and tetrahedra"  Journal of Computational Mathematics,
      // v. 27, no. 1, 2009, pp. 89-96.
    case QuadratureOrder::EIGHTH:
    {
      const unsigned int n_wts = 5;
      const Real wts[n_wts] = {Real(7.2157803838893584125545555244532310e-02L),
      ...
      // Another Dunavant rule.  This one has all positive weights.  This rule
      // has 19 points. The comparable conical product rule would have 25. It
      // was copied 23rd June 2008 from:
      // http://people.scs.fsu.edu/~burkardt/f_src/dunavant/dunavant.f90
      //
      // Additional precision obtained from the code in:
      // L. Zhang, T. Cui, and H. Liu. "A set of symmetric quadrature rules
      // on triangles and tetrahedra"  Journal of Computational Mathematics,
      // v. 27, no. 1, 2009, pp. 89-96.
    case QuadratureOrder::NINTH:
    {
      const unsigned int n_wts = 6;
      const Real wts[n_wts] = {Real(4.8567898141399416909620991253644315e-02L),
      ...
      // on triangles and tetrahedra"  Journal of Computational Mathematics,
      // v. 27, no. 1, 2009, pp. 89-96.
    case QuadratureOrder::TENTH:
    {
      const unsigned int n_wts = 6;
      const Real wts[n_wts] = {Real(4.5408995191376790047643297550014267e-02L),
      ...
      //
      // This 30-point, 11th-order rule was obtained by me [JWP] using the code
      // in
      //
      // Additional precision obtained from the code in:
      // L. Zhang, T. Cui, and H. Liu. "A set of symmetric quadrature rules
    ...
    case QuadratureOrder::ELEVENTH:
    {
      const unsigned int n_wts = 6;
      const Real wts[n_wts] = {Real(3.6089021198604635216985338480426484e-02L),
      ...
      const Real a[n_wts] = {Real(3.9355079629947969884346551941969960e-01L),
      ...
      // points. The comparable conical product rule would have 36 (ELEVENTH) or
      // 49 (TWELFTH).
      //
      // It was copied 23rd June 2008 from:
      // http://people.scs.fsu.edu/~burkardt/f_src/dunavant/dunavant.f90
      ...
      // on triangles and tetrahedra"  Journal of Computational Mathematics,
      // v. 27, no. 1, 2009, pp. 89-96.
    case QuadratureOrder::TWELFTH:
    {
      const unsigned int n_wts = 8;
      const Real wts[n_wts] = {Real(3.0831305257795086169332418926151771e-03L),
      ...
      const Real a[n_wts] = {Real(2.1317350453210370246856975515728246e-02L),
      ...
      // on triangles and tetrahedra"  Journal of Computational Mathematics,
      // v. 27, no. 1, 2009, pp. 89-96.
    case QuadratureOrder::THIRTEENTH:
    {
      const unsigned int n_wts = 9;
      const Real wts[n_wts] = {Real(3.3980018293415822140887212340442440e-02L),
      ...
      // on triangles and tetrahedra"  Journal of Computational Mathematics,
      // v. 27, no. 1, 2009, pp. 89-96.
    case QuadratureOrder::FOURTEENTH:
    {
      const unsigned int n_wts = 10;
      const Real wts[n_wts] = {Real(1.0941790684714445320422472981662986e-02L),
      ...
      const Real a[n_wts] = {Real(4.8896391036217863867737602045239024e-01L),
      ...
      const unsigned int permutation_ids[n_wts] = {3, 3, 3, 3, 3, 3, 6, 6, 6, 6}; // 42 total points
    ...
    case QuadratureOrder::FIFTEENTH:
    {
      const unsigned int n_wts = 11;
      const Real wts[n_wts] = {Real(2.4777380743035579804788826970198951e-02L),
      ...
      const Real a[n_wts] = {0.0, // 'a' parameter not used for origin
      ...
      // integration, and is thus unacceptable for our FEM calculations.
      //
      // This 55-point, 16th-order rule was obtained by me [JWP] using the code
      // in
      //
      // Additional precision obtained from the code in:
      // L. Zhang, T. Cui, and H. Liu. "A set of symmetric quadrature rules
      ...
      // it involves too many unknowns and the Jacobian is therefore singular
      // when attempting to improve the solution via Newton's method.
    case QuadratureOrder::SIXTEENTH:
    {
      const unsigned int n_wts = 12;
      const Real wts[n_wts] = {Real(2.2668082505910087151996321171534230e-02L),
      ...
      const Real a[n_wts] = {0.0, // 'a' parameter not used for centroid weight
      ...
      // a few more points.
    case QuadratureOrder::SEVENTEENTH:
    {
      const unsigned int n_wts = 12;
      const Real wts[n_wts] = {Real(1.7464603792572004485690588092246146e-02L),
      ...
      const Real a[n_wts] = {Real(2.8796825754667362165337965123570514e-01L),
      ...
      //       const Real p[15][4] = {
      // {                1./3.,                    0.,                    0.,
      // 0.033437199290803e+00 / 2.0}, // 1-perm {0.005658918886452e+00,
      // 0.497170540556774e+00,                    0., 0.005093415440507e+00
      // / 2.0}, // 3-perm {0.035647354750751e+00, 0.482176322624625e+00, 0.,
      // 0.014670864527638e+00 / 2.0}, // 3-perm {0.099520061958437e+00,
      // 0.450239969020782e+00,                    0., 0.024350878353672e+00
      // / 2.0}, // 3-perm {0.199467521245206e+00, 0.400266239377397e+00, 0.,
      // 0.031107550868969e+00 / 2.0}, // 3-perm {0.495717464058095e+00,
      // 0.252141267970953e+00,                    0., 0.031257111218620e+00
      // / 2.0}, // 3-perm {0.675905990683077e+00, 0.162047004658461e+00, 0.,
      // 0.024815654339665e+00 / 2.0}, // 3-perm {0.848248235478508e+00,
      // 0.075875882260746e+00,                    0., 0.014056073070557e+00
      // / 2.0}, // 3-perm {0.968690546064356e+00, 0.015654726967822e+00, 0.,
      // 0.003194676173779e+00 / 2.0}, // 3-perm {0.010186928826919e+00,
      // 0.334319867363658e+00, 0.655493203809423e+00, 0.008119655318993e+00
      // / 2.0}, // 6-perm {0.135440871671036e+00, 0.292221537796944e+00,
      // 0.572337590532020e+00, 0.026805742283163e+00 / 2.0}, // 6-perm
      // {0.054423924290583e+00, 0.319574885423190e+00, 0.626001190286228e+00,
      // 0.018459993210822e+00 / 2.0}, // 6-perm {0.012868560833637e+00,
      // 0.190704224192292e+00, 0.796427214974071e+00, 0.008476868534328e+00
      // / 2.0}, // 6-perm {0.067165782413524e+00, 0.180483211648746e+00,
      // 0.752351005937729e+00, 0.018292796770025e+00 / 2.0}, // 6-perm
      // {0.014663182224828e+00, 0.080711313679564e+00, 0.904625504095608e+00,
      // 0.006665632004165e+00 / 2.0}  // 6-perm
      //       };
      ...
      // Dunavant's 18th-order rule contains points outside the region and is
      // therefore unsuitable for our FEM calculations.  His 19th-order rule has
      // 73 points, compared with 100 points for a comparable-order conical
      // product rule.
      //
      // It was copied 23rd June 2008 from:
      // http://people.scs.fsu.edu/~burkardt/f_src/dunavant/dunavant.f90
    case QuadratureOrder::EIGHTTEENTH:
    case QuadratureOrder::NINETEENTH:
    {
      _points.resize(73);
      _weights.resize(73);
      ...
      const Real rule_data[17][4] = {
        ...
        {0.003611417848412e+00,
         ...
         0.003884876904981e+00 / 2.0}, // 6-perm
        {0.134466754530780e+00,
         0.307929983880436e+00,
         0.557603261588784e+00,
         0.025574160612022e+00 / 2.0}, // 6-perm
        {0.014446025776115e+00,
         0.264566948406520e+00,
         0.720987025817365e+00,
         0.008880903573338e+00 / 2.0}, // 6-perm
        {0.046933578838178e+00,
         0.358539352205951e+00,
         0.594527068955871e+00,
         0.016124546761731e+00 / 2.0}, // 6-perm
        {0.002861120350567e+00,
         0.157807405968595e+00,
         0.839331473680839e+00,
         0.002491941817491e+00 / 2.0}, // 6-perm
        {0.223861424097916e+00,
         0.075050596975911e+00,
         0.701087978926173e+00,
         0.018242840118951e+00 / 2.0}, // 6-perm
        {0.034647074816760e+00,
         0.142421601113383e+00,
         0.822931324069857e+00,
         0.010258563736199e+00 / 2.0}, // 6-perm
        {0.010161119296278e+00,
         0.065494628082938e+00,
         0.924344252620784e+00,
         0.003799928855302e+00 / 2.0} // 6-perm
      };
      ...
      // http://people.scs.fsu.edu/~burkardt/f_src/wandzura/wandzura.f90
    case QuadratureOrder::TWENTIETH:
    {
      ...
      const Real rule_data[19][4] = {
        ...
        {0.00534961818734e+00,
         0.06354966590835e+00,
         0.93110071590431e+00,
         0.2349170908575584e-02 / 2.0}, // 6-perm
        {0.00795481706620e+00,
         0.15710691894071e+00,
         0.83493826399309e+00,
         0.4465925754181793e-02 / 2.0}, // 6-perm
        {0.01042239828126e+00,
         0.39564211436437e+00,
         0.59393548735436e+00,
         0.6099566807907972e-02 / 2.0}, // 6-perm
        {0.01096441479612e+00,
         0.27316757071291e+00,
         0.71586801449097e+00,
         0.6891081327188203e-02 / 2.0}, // 6-perm
        {0.03856671208546e+00,
         0.10178538248502e+00,
         0.85964790542952e+00,
         0.7997475072478163e-02 / 2.0}, // 6-perm
        {0.03558050781722e+00,
         0.44665854917641e+00,
         0.51776094300637e+00,
         0.7386134285336024e-02 / 2.0}, // 6-perm
        {0.04967081636276e+00,
         0.19901079414950e+00,
         0.75131838948773e+00,
         0.1279933187864826e-01 / 2.0}, // 6-perm
        {0.05851972508433e+00,
         0.32426118369228e+00,
         0.61721909122339e+00,
         0.1725807117569655e-01 / 2.0}, // 6-perm
        {0.12149778700439e+00,
         0.20853136321013e+00,
         0.66997084978547e+00,
         0.1867294590293547e-01 / 2.0}, // 6-perm
        {0.14071084494394e+00,
         0.32317056653626e+00,
         0.53611858851980e+00,
         0.2281822405839526e-01 / 2.0} // 6-perm
      };
      ...
      // Copied on 3rd July 2008 from:
      // http://people.scs.fsu.edu/~burkardt/f_src/wandzura/wandzura.f90
      // case TWENTYFIRST: // fall through to 121 point conical product rule
      // below
    case QuadratureOrder::TWENTYSECOND:
    case QuadratureOrder::TWENTYTHIRD:
    case QuadratureOrder::TWENTYFOURTH:
    case QuadratureOrder::TWENTYFIFTH:
    {
      ...
      const Real rule_data[26][4] = {
        ...
        {0.00129235270444e+00,
         0.22721445215336e+00,
         0.77149319514219e+00,
         0.1248289199277397e-02 / 2.0}, // 6-perm
        {0.00539970127212e+00,
         0.43501055485357e+00,
         0.55958974387431e+00,
         0.3404752908803022e-02 / 2.0}, // 6-perm
        {0.00638400303398e+00,
         0.32030959927220e+00,
         0.67330639769382e+00,
         0.3359654326064051e-02 / 2.0}, // 6-perm
        {0.00502821150199e+00,
         0.09175032228001e+00,
         0.90322146621800e+00,
         0.1716156539496754e-02 / 2.0}, // 6-perm
        {0.00682675862178e+00,
         0.03801083585872e+00,
         0.95516240551949e+00,
         0.1480856316715606e-02 / 2.0}, // 6-perm
        {0.01001619963993e+00,
         0.15742521848531e+00,
         0.83255858187476e+00,
         0.3511312610728685e-02 / 2.0}, // 6-perm
        {0.02575781317339e+00,
         0.23988965977853e+00,
         0.73435252704808e+00,
         0.7393550149706484e-02 / 2.0}, // 6-perm
        {0.03022789811992e+00,
         0.36194311812606e+00,
         0.60782898375402e+00,
         0.7983087477376558e-02 / 2.0}, // 6-perm
        {0.03050499010716e+00,
         0.08355196095483e+00,
         0.88594304893801e+00,
         0.4355962613158041e-02 / 2.0}, // 6-perm
        {0.04595654736257e+00,
         0.14844322073242e+00,
         0.80560023190501e+00,
         0.7365056701417832e-02 / 2.0}, // 6-perm
        {0.06744280054028e+00,
         0.28373970872753e+00,
         0.64881749073219e+00,
         0.1096357284641955e-01 / 2.0}, // 6-perm
        {0.07004509141591e+00,
         0.40689937511879e+00,
         0.52305553346530e+00,
         0.1174996174354112e-01 / 2.0}, // 6-perm
        {0.08391152464012e+00,
         0.19411398702489e+00,
         0.72197448833499e+00,
         0.1001560071379857e-01 / 2.0}, // 6-perm
        {0.12037553567715e+00,
         0.32413434700070e+00,
         0.55549011732214e+00,
         0.1330964078762868e-01 / 2.0}, // 6-perm
        {0.14806689915737e+00,
         0.22927748355598e+00,
         0.62265561728665e+00,
         0.1415444650522614e-01 / 2.0}, // 6-perm
        {0.19177186586733e+00,
         0.32561812259598e+00,
         0.48261001153669e+00,
         0.1488137956116801e-01 / 2.0} // 6-perm
      };
      ...
      // Copied on 3rd July 2008 from:
      // http://people.scs.fsu.edu/~burkardt/f_src/wandzura/wandzura.f90
    case QuadratureOrder::TWENTYSIXTH:
    case QuadratureOrder::TWENTYSEVENTH:
    case QuadratureOrder::TWENTYEIGHTH:
    case QuadratureOrder::TWENTYNINTH:
    case QuadratureOrder::THIRTIETH:
    {
      ...
      const Real rule_data[36][4] = {
        ...
        {0.00092537119335e+00,
         0.41529527091331e+00,
         0.58377935789334e+00,
         0.9989056850788964e-03 / 2.0}, // 6-perm
        {0.00138592585556e+00,
         0.06118990978535e+00,
         0.93742416435909e+00,
         0.4628508491732533e-03 / 2.0}, // 6-perm
        {0.00368241545591e+00,
         0.16490869013691e+00,
         0.83140889440718e+00,
         0.1234451336382413e-02 / 2.0}, // 6-perm
        {0.00390322342416e+00,
         0.02503506223200e+00,
         0.97106171434384e+00,
         0.5707198522432062e-03 / 2.0}, // 6-perm
        {0.00323324815501e+00,
         0.30606446515110e+00,
         0.69070228669389e+00,
         0.1126946125877624e-02 / 2.0}, // 6-perm
        {0.00646743211224e+00,
         0.10707328373022e+00,
         0.88645928415754e+00,
         0.1747866949407337e-02 / 2.0}, // 6-perm
        {0.00324747549133e+00,
         0.22995754934558e+00,
         0.76679497516308e+00,
         0.1182818815031657e-02 / 2.0}, // 6-perm
        {0.00867509080675e+00,
         0.33703663330578e+00,
         0.65428827588746e+00,
         0.1990839294675034e-02 / 2.0}, // 6-perm
        {0.01559702646731e+00,
         0.05625657618206e+00,
         0.92814639735063e+00,
         0.1900412795035980e-02 / 2.0}, // 6-perm
        {0.01797672125369e+00,
         0.40245137521240e+00,
         0.57957190353391e+00,
         0.4498365808817451e-02 / 2.0}, // 6-perm
        {0.01712424535389e+00,
         0.24365470201083e+00,
         0.73922105263528e+00,
         0.3478719460274719e-02 / 2.0}, // 6-perm
        {0.02288340534658e+00,
         0.16538958561453e+00,
         0.81172700903888e+00,
         0.4102399036723953e-02 / 2.0}, // 6-perm
        {0.03273759728777e+00,
         0.09930187449585e+00,
         0.86796052821639e+00,
         0.4021761549744162e-02 / 2.0}, // 6-perm
        {0.03382101234234e+00,
         0.30847833306905e+00,
         0.65770065458860e+00,
         0.6033164660795066e-02 / 2.0}, // 6-perm
        {0.03554761446002e+00,
         0.46066831859211e+00,
         0.50378406694787e+00,
         0.3946290302129598e-02 / 2.0}, // 6-perm
        {0.05053979030687e+00,
         0.21881529945393e+00,
         0.73064491023920e+00,
         0.6644044537680268e-02 / 2.0}, // 6-perm
        {0.05701471491573e+00,
         0.37920955156027e+00,
         0.56377573352399e+00,
         0.8254305856078458e-02 / 2.0}, // 6-perm
        {0.06415280642120e+00,
         0.14296081941819e+00,
         0.79288637416061e+00,
         0.6496056633406411e-02 / 2.0}, // 6-perm
        {0.08050114828763e+00,
         0.28373128210592e+00,
         0.63576756960645e+00,
         0.9252778144146602e-02 / 2.0}, // 6-perm
        {0.10436706813453e+00,
         0.19673744100444e+00,
         0.69889549086103e+00,
         0.9164920726294280e-02 / 2.0}, // 6-perm
        {0.11384489442875e+00,
         0.35588914121166e+00,
         0.53026596435959e+00,
         0.1156952462809767e-01 / 2.0}, // 6-perm
        {0.14536348771552e+00,
         0.25981868535191e+00,
         0.59481782693256e+00,
         0.1176111646760917e-01 / 2.0}, // 6-perm
        {0.18994565282198e+00,
         0.32192318123130e+00,
         0.48813116594672e+00,
         0.1382470218216540e-01 / 2.0} // 6-perm
      };
...
chi_math::QuadratureTriangle::dunavant_rule(const double rule_data[][4], const unsigned int n_pts)
{
  ...
  // points.  A zero in two of the first 3 columns implies the point is a
  // 1-permutation (centroid). A zero in one of the first 3 columns implies the
  // point is a 3-permutation. Otherwise each point is assumed to be a
  // 6-permutation.
                                             ...
                                             unsigned int n_wts)
{
  ...
  for (unsigned int p = 0; p < n_wts; ++p)
  {
    switch (permutation_ids[p])
    {
      ...
      default:
        throw std::invalid_argument(std::string(__FUNCTION__) + "Unknown permutation id: " +
                                    std::to_string(permutation_ids[p]) + "!");
    }
  }
}

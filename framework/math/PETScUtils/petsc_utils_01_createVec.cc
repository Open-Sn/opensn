...
\endcode*/
Vec
chi_math::PETScUtils::CreateVector(int64_t local_size, int64_t global_size)
{
  Vec x;
  VecCreate(PETSC_COMM_WORLD, &x);
  VecSetType(x, VECMPI);
...
\endcode*/
void
chi_math::PETScUtils::CreateVector(Vec& x, int64_t local_size, int64_t global_size)
{
  VecCreate(PETSC_COMM_WORLD, &x);
  VecSetType(x, VECMPI);
  VecSetSizes(x, local_size, global_size);
...
\endcode*/
Vec
chi_math::PETScUtils::CreateVectorWithGhosts(int64_t local_size,
                                             int64_t global_size,
                                             int64_t nghosts,

...
chi_math::PETScUtils::CreateCommonKrylovSolverSetup(Mat ref_matrix,
                                                    ...
                                                    int64_t in_maximum_iterations)
{
  ...
  KSPMonitorSet(setup.ksp, &chi_math::PETScUtils::KSPMonitorRelativeToRHS, nullptr, nullptr);
...
PetscErrorCode
chi_math::PETScUtils::KSPMonitorRelativeToRHS(KSP ksp, PetscInt n, PetscReal rnorm, void*)
{
  Vec Rhs;
  KSPGetRhs(ksp, &Rhs);
  double rhs_norm;
 ...
 * right-hand side norm.*/
PetscErrorCode
chi_math::PETScUtils::KSPMonitorStraight(KSP ksp, PetscInt n, PetscReal rnorm, void*)
{
  // Get solver name
  const char* ksp_name;
  KSPGetOptionsPrefix(ksp, &ksp_name);

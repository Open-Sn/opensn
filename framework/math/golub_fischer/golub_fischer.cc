...
{
  Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << "Getting Discrete Scattering Angles" << '\n';

  for (int m = 0; m < mell.size(); m++)
  {
    Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << "Moment " << m << " " << mell[m];
  }
  ...
  Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << "a,b,c:\n";
  for (int j = 0; j < 2 * n; j++)
  {
    a[j] = 0.0;
    b[j] = j / (2.0 * j + 1);
    c[j] = (j + 1.0) / (2 * j + 1);
    Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << a[j] << " " << b[j] << " " << c[j] << " \n";
  }
  ...
  for (int i = 0; i < n; i++)
  {
    Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2)
      << "i " << xn_wn_[i].first << " " << xn_wn_[i].second << '\n';
  }

  Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << "Done" << '\n';
...
{
  Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << "MCA Start" << '\n';

  int N = in_mell.size() - 1;
  int n = (N + 1) / 2;

  Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << "N " << N << " n " << n << '\n';
  Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << "alpha, beta" << '\n';
  ...
  Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << 0 << " " << alpha_[0] << " " << beta_[0] << "\n";
...
{
  Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << "RootsOrtho Start" << '\n';

  int maxiters = 1000;
  double tol = 1.0e-6;
  double adder = 0.999 * 2 / std::max(N - 1, 1);

  Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << "Check 1: Init guess" << '\n';
  ...
  {
    xn[i] = -0.999 + i * adder;
    Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << "x[" << i << "]=" << xn[i] << "\n";
  }

  Tvecdbl norm;
  norm.resize(N + 1, 0.0);
  Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << "Check 2 " << in_beta[0] << '\n';
  norm[0] = in_beta[0];
  Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << "Check 3a norms" << '\n';
  Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << norm[0] << '\n';
  for (int i = 1; i < (N + 1); i++)
  {
    norm[i] = in_beta[i] * norm[i - 1];
    Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << norm[i] << '\n';
  }

  Chi::log.Log(Logger::LOG_LVL::LOG_0VERBOSE_2) << "Check 3" << '\n';

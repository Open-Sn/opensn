...
:py:class:`pyopensn.solver.UncollidedProblem`. Supplying it enables the
...
:py:class:`pyopensn.solver.UncollidedProblem` computes and stores the
uncollided flux moments. A subsequent
...
The uncollided calculation is executed when
:py:class:`pyopensn.solver.UncollidedProblem` is constructed. It does not
require a separate solver object:

Source-point progress is reported every 5 percent by default. Each update
includes the number of completed source points, elapsed time, and estimated
remaining time. Set ``progress_interval`` to another percentage, or to zero to
...
* isotropic :py:class:`pyopensn.source.VolumetricSource` objects,
* both source types in the same calculation.

Every explicit point source requires a corresponding entry in ``near_source``.
The two lists are matched by position.

Volumetric sources are integrated by replacing each subscribing source cell
with point sources at volume-quadrature points:

.. math::

   Q_{g,q} = q_g(\mathbf{x}_q) J_q w_q.

This preserves the quadrature approximation to the integrated source and
supports spatially varying ``VectorSpatialFunction`` sources. Source functions
and source activity are evaluated at steady-state time zero.

``volumetric_source_quadrature_order=2`` is the default and uses multiple
points per cell. Order 1 uses one volume-weighted cell-centroid point and is
appropriate for cellwise-uniform sources when generation cost is more
important than resolving within-cell source variation.
...
sources and folds attenuation paths back through the physical mesh. With
:math:`N` reflecting symmetry planes, each physical source point produces
:math:`2^N-1` image contributions.
...
Volumetric near-source region
-----------------------------

By default, only the containing source cell is ray traced for each generated
volumetric quadrature point. The remaining cells use the less expensive
uncollided bulk sweep.

The optional ``volumetric_near_source`` argument supplies one logical volume
shared by every generated volumetric quadrature point:
...
.. code-block:: python
   ...
   uncollided = UncollidedProblem(
       ...
       volumetric_near_source=raytrace_region,
       scattering_order=scattering_order,
       file_name="uncollided_flux.h5",
   )

Every generated quadrature point must lie inside this logical volume. A larger
region generally improves accuracy because more of the singular point-source
field is evaluated analytically, but its cost is paid separately for every
source quadrature point.

In the included small regression problems, changing from source-cell-only to
whole-domain ray tracing reduced the sampled relative errors from approximately
1.91% to 0.039% in 2D and from 10.4% to 0.41% in 3D. Runtime increased by
approximately 1.4--1.6 times. These values characterize those tests rather than
providing general error bounds.
...
The field functions returned directly by
:py:class:`pyopensn.solver.UncollidedProblem` contain the uncollided component
:math:`\Phi^u`.
...
outflow when reporting the combined problem balance. For reflected image
sources, independently projected image fields can leave a small global current
inconsistency. The generator reports the integrated and conservative effective
outflows and stores the conservative value used for combined balance
accounting; this correction does not rescale the uncollided flux moments.

By default, reflected image sources use the bulk radial sweep outside the
physical source's near-source region. Set
``project_reflected_image_sources=True`` to project each folded reflected path
directly at the volume quadrature points in every cell. This removes accumulated
bulk-sweep error from reflected paths, but its cost scales with the product of
the image-source count and global cell count and is therefore intended mainly
for accuracy studies. Set ``reflected_image_projection_threads`` to project
independent cells concurrently while retaining a single MPI rank.
When direct image projection is active, the printed integrated-current
diagnostic includes only nonprojected sources; the stored conservative outflow
continues to account for the complete uncollided solution.
